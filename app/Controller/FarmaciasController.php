...
class FarmaciasController extends AppController {
	...
	public function add() {
		if (!empty($this->request->data)) {
			...
			if ($this->Farmacia->save($this->request->data)) {
				...
				$this->Session->setFlash("El post no ha podido ser guardado.");
	...
	public function admin_edit($id = null) {
		...
		if (!$farmacia) {	
			...
			if ($this->request->is(array('post', 'put'))) {
					...
					if ($user_id != $this->Auth->user('id')) {
						$this->Session->setFlash(__('No tiene permiso para una farmacia que no es la suya'));
						}
		...
		$oferta = $this->Oferta->find('all', array('conditions' => array('Oferta.id_farmacia' => $id)
		));
